...
function init_gui(player, ignoretech)
	...
	if (not player.gui.top["blueprint-string-button"]) then
		player.gui.top.add{type="button", name="blueprint-string-button", caption={"main-button"}}
	end
end
...
function expand_gui(player)
	...
	if (frame) then
		...
		frame.add{type="button", name="blueprint-string-load", caption={"load-string"}}
		frame.add{type="button", name="blueprint-string-save-as", caption={"save-as"}}
...
function save_blueprint(player, stack, filename)
	...
	game.makefile("blueprint-string/" .. filename .. ".txt", data)
...
function prompt_for_filename(player)
	...
	frame.add{type="label", caption="Save as:"}
	frame.add{type="textfield", name="blueprint-string-filename"}
	local line2 = frame.add{type="flow", direction="horizontal"}
	line2.add{type="button", name="blueprint-string-filename-save", caption="Save"}
	line2.add{type="button", name="blueprint-string-filename-cancel", caption="Cancel"}
end
